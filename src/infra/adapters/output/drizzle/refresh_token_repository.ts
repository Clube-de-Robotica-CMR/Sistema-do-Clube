...
export class DrizzleRefreshTokensRepository implements RefreshTokensRepository {
  async save(token: CreateRefreshTokenDTO): Promise<void> {
  ...
  async get_by_token(token: string): Promise<RefreshToken | null> {
    const result = await db
      .select()
  ...
  async delete_by_token(token: string): Promise<void> {
    const result = await db
  ...
  async delete_by_user_id(userId: string): Promise<void> {
    const result = await db
        .delete(refresh_tokens_table)
